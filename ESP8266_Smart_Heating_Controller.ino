...
void handle_wifi_tcp_requests() {
  ...
  //Manage concurrent / parallel connections
  if (Web_server.hasClient()) {
  ...
  if (http_request_url.indexOf(FPSTR(GUI_Root_Endpoint)) != -1) {
    ...
    Web_server_client.println(render_html_simple_14_select_string(F("/operation"), "GET", "MongoDB_Query", "Exception", "Hardware Watchdog", "Software Watchdog", "External System", "Software/System restart", "Power On", "ESP_Started", "ESP_Restart", "Relay_Start", "Relay_Stop", "Auto_Mode_Start", "Relay_Auto_Start", "Relay_Auto_Stop", "ESP_FW_Update", "Submit"));
    ...
    Web_server_client.print(F("<br>Serial_Debug_Speed : ") + String(Serial_Debug_Speed) + " baud");
    ...
    Web_server_client.println(F("FW_Filestore_file : ") + ESP_FW_Filestore_file);
    ...
    Web_server_client.println(F("<br>Low Memory Counter : ") + String(Low_Memory_counter));
    Free_Heap = ESP.getFreeHeap();
    if (Free_Heap < 30000) {
    ...
    if (query_param.indexOf(F("DNS_lookup")) != -1) {
      ...
      if (tcp_mongo_client.connect(MongoDB_Server, MongoDB_Server_port)) {
        ...
        if (param == "Exception" || param == "Hardware+Watchdog" || param == "Software+Watchdog" || param == "External+System" || param == "Software%2FSystem+restart" || param == "Power+On") {
          Mongo_request_full_payload = F("GET /iot/audit/?filter_ResetReason=") + param + F("&limit=-2000 HTTP/1.1\r\n\r\n");  //skip=5 can be an option
        } else if (param == "ESP_Started" || param == "ESP_Restart" || param == "Relay_Start" || param == "Relay_Stop" || param == "Auto_Mode_Start" || param == "Relay_Auto_Start" || param == "Relay_Auto_Stop" || param == "ESP_FW_Update") {
          Mongo_request_full_payload = F("GET /iot/audit/?filter_Operation=") + param + F("&limit=-2000 HTTP/1.1\r\n\r\n");  //skip=5 can be an option
        } else {
          Serial.println(F("WARN: Unknown command: ") + String(param));
          Web_server_client.println(F("Unknown command: ") + String(param) + F("<br>"));
...
bool relay_start_command() {
  ...
  if ((relay_response_timestamp - relay_request_arrival_timestamp) > 250) {
    Serial.println(F("WARN: Slow Response"));
  }
...
void free_mem_check() {
  ...
  if (Free_Heap < 30000) {
    ...
    Low_Memory_counter = Low_Memory_counter + 1;
  }
  ...
  if (Outbound_History.length() > Outbound_History_max_bytes) {  //limit String size
    ...
    Serial.println("DEBUG: " + String(FPSTR(Outbound_hist_purge)));
  }
  ...
  if (Relay_History.length() > Relay_History_max_bytes) {  //limit String size
    ...
    Serial.println("DEBUG: " + String(FPSTR(Relay_hist_purge)));
  }
}
...
void ESP_FW_Update() {
  ...
  //C:\Users\<user>\AppData\Local\Temp\arduino\sketches\7095592A2D49FD0DC19C715F54AC6A6C\ESP8266_Smart_Heating_Controller.ino.bin
  t_httpUpdate_return update_result;
  update_result = ESPhttpUpdate.update(updateclient_f_server, ESP_FW_Filestore_Server, ESP_FW_Filestore_Port, ESP_FW_Filestore_file);
...
void recurring_task_4() {
  ...
  mail_content = F("Timestamp : ") + String(timeClient.getFormattedDate()) + " Hostname : " + WiFi.hostname() + " ESP Auto Update Initiated";
  send_mail_now();
  ESP_FW_Update();
...
String render_html_simple_14_select_string(String form_action, String form_method, String form_label_name, String form_parameter_1, String form_parameter_2, String form_parameter_3, String form_parameter_4, String form_parameter_5, String form_parameter_6, String form_parameter_7, String form_parameter_8, String form_parameter_9, String form_parameter_10, String form_parameter_11, String form_parameter_12, String form_parameter_13, String form_parameter_14, String button_text) {
  String rendered_form = "";
  rendered_form = F("<form action=\"") + String(form_action) + F("\" method=\"") + String(form_method) + F("\"><label>") + String(form_label_name) + F("</label>");
  ...
  rendered_form = rendered_form + F("<option value=\"") + String(form_parameter_7) + F("\">") + String(form_parameter_7) + F("</option>");
  rendered_form = rendered_form + F("<option value=\"") + String(form_parameter_8) + F("\">") + String(form_parameter_8) + F("</option>");
  rendered_form = rendered_form + F("<option value=\"") + String(form_parameter_9) + F("\">") + String(form_parameter_9) + F("</option>");
  rendered_form = rendered_form + F("<option value=\"") + String(form_parameter_10) + F("\">") + String(form_parameter_10) + F("</option>");
  rendered_form = rendered_form + F("<option value=\"") + String(form_parameter_11) + F("\">") + String(form_parameter_11) + F("</option>");
  rendered_form = rendered_form + F("<option value=\"") + String(form_parameter_12) + F("\">") + String(form_parameter_12) + F("</option>");
  rendered_form = rendered_form + F("<option value=\"") + String(form_parameter_13) + F("\">") + String(form_parameter_13) + F("</option>");
  rendered_form = rendered_form + F("<option value=\"") + String(form_parameter_14) + F("\">") + String(form_parameter_14) + F("</option>");
